/**
 * ═══════════════════════════════════════════════════════════════════════════
 * ADVISOR MEETING SUMMARY — the meeting-note engine.
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * WHAT IT IS
 *   A PRIVATE child of advisorAssistant (js-meta.xml: isExposed=false — it cannot
 *   be dropped on a Flow screen or record page on its own). It owns the entire
 *   life of a meeting note: generate it, edit it, save drafts, publish, version,
 *   delete — plus the AI-generated to-do list derived from the same meeting.
 *
 * MOUNTED TWICE
 *   The parent renders this component in two places with `embedded-layout`:
 *     data-role="summary"  → the Summary tab
 *     data-role="todos"    → the To-Do's tab, with active-ms-section="todos"
 *   They are INDEPENDENT INSTANCES with independent state. `activeMsSection`
 *   decides which half of the UI a given instance shows.
 *
 * TWO LAYOUTS
 *   `embeddedLayout = true`  (how the parent uses it) — body only. The parent
 *      supplies the header, the Configuration column and the save bar, and drives
 *      this component through the imperative @api methods listed below.
 *   `embeddedLayout = false` — standalone: renders its own left config panel,
 *      segment bar and save bar. Kept for direct use and for debugging.
 *
 * PUBLIC SURFACE (what the parent may call / listen to)
 *   Commands  generateSummary() · generateTodos() · saveNote() · saveNoteAndExit()
 *             publishNote() · publishNoteAndExit() · deleteNote()
 *             regenerateSummaryWithInstructions(text) · resetState()
 *   State     msCanPublish · msCanDelete · msHasUnpublishedSummary
 *             msHasUnsavedChanges
 *   Events    summarychange {summaryHtml} · todoschange {todos, acceptedCount}
 *             msstatechange {8 booleans — emitted on EVERY renderedCallback, and
 *                            the authority the parent's footer renders from}
 *             flowoutput {attributeName, attributeValue} · flownext
 *
 * WHY flowoutput INSTEAD OF FlowAttributeChangeEvent
 *   A nested component cannot reach the Flow runtime — only the component actually
 *   on the Flow screen can. So this file has no `lightning/flowSupport` import;
 *   every output goes through `_flowOut()`, and advisorAssistant re-emits it as a
 *   real Flow event. This is the single deliberate difference from the
 *   wealthPlanHelper monolith it was forked from.
 *
 * NOTE STORAGE MODEL
 *   MeetingNote__c holds the current note (Note__c = HTML, NoteMarkdown__c =
 *   Markdown mirror for the client web app, Published__c). Publishing freezes it;
 *   editing a published note creates a MeetingNoteVersion__c through the separate
 *   `createVersionFlowApiName` Flow rather than overwriting history.
 *
...
export default class AdvisorMeetingSummary extends NavigationMixin(LightningElement) {
    // ── Inputs ───────────────────────────────────────────────────────────────
    @api recordId;
    ...
    // setters the attributes the parent passes are silently ignored by LWC and the
    // child builds its flow payload from permanently-empty local state.
    ...
    @api get selectedMeetingType() { return this._selectedMeetingType; }
    set selectedMeetingType(v) { this._selectedMeetingType = v || null; }

    @api get hasTodos() { return this._hasTodos; }
    set hasTodos(v) { if (v !== undefined) this._hasTodos = !!v; }

    get showInternalLeftPanel() { return !this.embeddedLayout; }
    get showInternalActionBar() { return !this.embeddedLayout; }
    // Parent-owned tab bar already differentiates Meeting Summary vs To-Do's —
    // hide the internal Summary | To-Do's segmented control entirely when embedded.
    get showMsSegmentBar() {
        if (this.embeddedLayout) return false;
        return this._hasTodos;
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //
    // Commands come DOWN as imperative @api methods (below); results go UP as
    // CustomEvents. Each command is a thin alias for the internal handler, so the
    // same code path serves both the parent's footer and this component's own UI
    // in standalone mode — there is only ever one implementation of "publish".
    //
    // Changing a signature here is a breaking change for advisorAssistant.html and
    // for `_msChild(role)` in advisorAssistant.js. Grep both before you touch one.

    // Imperative bridges for the parent's consolidated footer.
    @api publishNote()        { return this.handleShowPublishConfirm(); }
    @api publishNoteAndExit() { return this.handlePublishAndExit(); }
    // Save the note without publishing — used by the parent footer's Save & Exit,
    // which must persist before navigating away.
    @api saveNote()           { return this.handleSaveSummary(); }
    @api saveNoteAndExit()    { return this.handleSaveAndExit(); }
    @api deleteNote()         { return this.handleShowDeleteNoteConfirm(); }
    // Parent invokes this from its "Regenerate with Instructions" card.
    @api regenerateSummaryWithInstructions(instructions) {
        this._summaryInstructions = instructions || '';
        return this.handleRegenerateSummary();
    }
    ...
    // Read-only state the parent's footer uses to decide which buttons to show.
    @api get msCanPublish() {
        return this.isSummaryTab && !this.noEventSelected && this.hasSummaryForEvent && !this.isNotePublished && !this._summaryGenerating;
    }
    @api get msCanDelete() { return this.canDeleteNote; }

    /** True when a summary exists for this event that has NOT been published yet.
     *  The parent uses this to offer "save before regenerating" instead of silently
     *  destroying an unpublished draft. */
    @api get msHasUnpublishedSummary() {
        return this.hasSummaryForEvent && !this.isNotePublished;
    }
    /** True when the in-memory draft differs from what was last persisted. This is the
     *  same signal the internal save bar branches on, so the parent footer can mirror it. */
    @api get msHasUnsavedChanges() { return this.canRevertSummary; }

    /**
     * Push this component's note state up to the parent, so its footer knows which
     * buttons to show.
     *
     * Called from `renderedCallback` — i.e. potentially on every render — so it
     * diffs against `_lastMsState` and only dispatches when a value actually
     * changed. Without that guard, each emit would re-render the parent, which
     * re-renders this child, which emits again.
     *
     * This event is the AUTHORITY for the parent's footer. advisorAssistant has its
     * own `hasSummaryForEvent` / `isNotePublished` getters left over from before the
     * split, but their backing fields are never written there — they are stale and
     * must not be used to gate actions.
     *
     * @fires msstatechange {canPublish, canDelete, hasSummary, isPublished,
     *                       isGenerating, isSaving, hasUnpublishedSummary,
     *                       hasUnsavedChanges}
     */
    _lastMsState = null;
    _emitMsState() {
        const state = {
            canPublish:  this.msCanPublish,
            canDelete:   this.msCanDelete,
            hasSummary:  this.hasSummaryForEvent,
            isPublished: this.isNotePublished,
        ...
        if (changed) {
            this._lastMsState = state;
            this.dispatchEvent(new CustomEvent('msstatechange', { detail: state }));
        }
    }

    // Imperative entry points invoked by the parent's dynamic primary CTA.
    @api generateSummary() { return this.handleGenerateSummaryDirect(); }
    @api generateTodos()   {
        // Force the child into todos mode so the UI actually renders the todos
        // accordion once the Apex call returns and _meetingTodos populates.
        this._activeMsSection = 'todos';
        this._hasTodos = true;
        return this.handleGenerateTodosDirect();
    }

    // Parent (advisorAssistant) drives sub-section via `active-ms-section="todos"`.
    // Without this @api hook the attribute binding hit a private @track and was ignored,
    // so _hasTodos never flipped on and the todos accordion stayed hidden.
    @api get activeMsSection() { return this._activeMsSection; }
    set activeMsSection(v) {
        const s = v || 'meeting';
        if (s === this._activeMsSection) return;
        this._activeMsSection = s;
        if (s === 'todos') this._hasTodos = true;
    }
    ...
    @track _wpSaveErrors            = null;  // string[] — section-level errors returned from partial save
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // @wire — PICKLIST METADATA (17 adapters)
    // ═══════════════════════════════════════════════════════════════════════
    //
    ...
    // until the object info resolves. Results are normalised to {label, value}.
    //
    // Unlike the parent's copy, these handlers actively push their results into the
    // memoised render model (`_refreshSelectOptionsMap` + `_refreshReviewSections`)
    // rather than relying on a getter recomputing per render.
    //
    // These belong to the WEALTH-PLAN half of this file (see the legacy banner
    ...
    wiredGoalValues({ data }) {
        if (data) {
            this._goalPicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }
    ...
    wiredStatusValues({ data }) {
        if (data) {
            this._statusPicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }
    ...
    wiredIncomeTypeValues({ data }) {
        if (data) {
            this._incomeTypePicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }
    ...
    wiredALTypeValues({ data }) {
        if (data) {
            this._alTypePicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }
    ...
    wiredAssetTypeValues({ data }) {
        if (data) {
            this._assetTypePicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }
    ...
    wiredAssetCategoryValues({ data }) {
        if (data) {
            this._assetCategoryPicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }
    ...
    wiredMilestoneYearValues({ data }) {
        if (data) {
            this._milestoneYearPicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }
    ...
    wiredStockClassValues({ data }) {
        if (data) {
            this._stockClassPicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }
    ...
    wiredSustPref({ data }) {
        if (data) {
            this._sustPrefPicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }
    ...
    wiredSustStrat({ data }) {
        if (data) {
            this._sustStratPicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }
    ...
    wiredSustShare({ data }) {
        if (data) {
            this._sustSharePicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }
    ...
    wiredSustImpact({ data }) {
        if (data) {
            this._sustImpactPicklistOptions = data.values.map(v => ({ label: v.label, value: v.value }));
            this._refreshSelectOptionsMap(); this._refreshReviewSections();
        }
    }

    /** Cached picklist options map — rebuilt only when wire data changes, not on every render */
    _cachedSelectOptionsMap = {};

    _refreshSelectOptionsMap() {
        this._cachedSelectOptionsMap = {
    ...
    get todoPriorityCounts() {
        ...
        for (const t of this._meetingTodos) {
            const p = t.Priority;
            if (p === 'High') high++;
            else if (p === 'Low') low++;
            else normal++;
    ...
    get showMsSaveBar()       {
        // Hide the internal Publish / Publish & Exit / Delete Note strip when embedded —
        // the parent (advisorAssistant) renders those buttons in its consolidated footer instead.
        if (this.embeddedLayout) return false;
        return this.isSummaryTab && !this.noEventSelected && this.hasSummaryForEvent && !this.isNotePublished && !this._summaryGenerating;
    }
    ...
    get summaryTabStyle()    { return this._activeTab === 'summary'    ? AdvisorMeetingSummary._TAB_ACTIVE_STYLE : AdvisorMeetingSummary._TAB_INACTIVE_STYLE; }
    get wealthPlanTabStyle() { return this._activeTab === 'wealthplan' ? AdvisorMeetingSummary._TAB_ACTIVE_STYLE : AdvisorMeetingSummary._TAB_INACTIVE_STYLE; }
    get existingTabStyle()   { return this._activeTab === 'existing'   ? AdvisorMeetingSummary._TAB_ACTIVE_STYLE : AdvisorMeetingSummary._TAB_INACTIVE_STYLE; }
    ...
    get meetingSubtabClass() { return this._activeMsSection === 'meeting' ? AdvisorMeetingSummary._SUBTAB_ACTIVE : AdvisorMeetingSummary._SUBTAB_INACTIVE; }
    get todosSubtabClass()   { return this._activeMsSection === 'todos'   ? AdvisorMeetingSummary._SUBTAB_ACTIVE : AdvisorMeetingSummary._SUBTAB_INACTIVE; }
    ...
    /**
     * Switch the component to a different calendar Event. The single most
     * state-heavy operation in this file — roughly twenty fields are reset.
     *
     * The design goal is that switching events is non-destructive and reversible
     * within a session, so it is a save/reset/restore cycle, not a plain reset:
     *
     *   1. CACHE OUT — stash the outgoing event's to-dos under its Id.
     *   2. RESET     — clear every piece of event-scoped state (draft HTML, brief,
     *                  instructions, edit modes, file selection, uploads).
     *   3. RESTORE   — for the incoming event, prefer this session's caches
     *                  (`_savedNoteCache`, `_meetingTypeCache`,
     *                  `_selectedTaskIdsCache`, `_meetingTodosCache`), then fall
     *                  back to what the Flow queried (MeetingNote__c, the Event's
     *                  own MeetingType__c).
     *   4. DEFAULT   — auto-select up to 3 artifact files, auto-open the accordions
     *                  that have content, auto-open Meeting Category when no type
     *                  is set, collapse the left panel if a summary already exists.
     *   5. LOAD      — kick off version history for the new event.
     *
     * The `Promise.resolve().then(...)` blocks are not cosmetic: `artifactFiles`
     * and the accordion predicates are getters over reactive inputs that have not
     * settled yet at this point in the call, so those defaults must be computed on
     * the microtask after the state assignments above.
     *
     * @param {string|null} newId Event Id, or null to deselect
     */
    ...
    async handleRegenerateSummary() {
        ...
        try {
            ...
            // Always emit — success or failure — so the parent's bg-job flag reliably clears.
            this.dispatchEvent(new CustomEvent('summarychange', {
                bubbles: false,
                detail: { summaryHtml: this._currentNoteHtml || '' }
            }));
        }
    }
     ...
     *
     * Two routes, chosen by `_creatingNewVersion`:
     *   - editing a PUBLISHED note → `createVersionFlowApiName`, which writes a new
     *     MeetingNoteVersion__c and repoints the parent MeetingNote__c, so published
     *     history is never overwritten;
     *   - otherwise → `saveSummaryFlowApiName`, a plain upsert of MeetingNote__c.
     * With no save Flow configured it degrades to an in-memory save and reports
     * success, so the component stays usable in an org that has not wired the Flows.
     *
     * Both routes send the note twice — `noteHtml` (what the advisor edits) and
     * `noteMarkdown` (the mirror the client web app reads) — plus the same pair for
     * the brief.
     *
     * @returns {Promise<boolean>} true ONLY if the note actually reached the server.
     *   Errors are surfaced as a toast, never thrown, so any caller that destroys
     *   the draft afterwards (save-before-regenerate, Save & Exit) MUST check this
     *   return value rather than assuming completion. Note the monolith's version of
     *   this method returns undefined — do not copy code back from it.
     */
    ...
    /**
     * Generate a meeting summary. The main AI entry point of this component, and
     * what the parent's "Generate Meeting Summary" CTA ultimately calls.
     *
     * Assembles the Flow payload from everything the advisor configured — event,
     * primary member, up to three uploaded/selected documents, free-text context,
     * meeting category, and whether to-dos were requested — then calls
     * `generateSummary` Apex, which runs `meetingSummaryFlowApiName` server-side.
     *
     * The response is accepted in two shapes, because the Flow was split partway
     * through the project's life: either plain summary HTML, or JSON
     * `{summary, brief}` from the older combined Flow. Anything that is not valid
     * JSON is treated as HTML.
     *
     * Brief and to-dos are then fired as SEPARATE async calls rather than awaited,
     * so the summary paints as soon as it arrives instead of waiting on all three.
     *
     * Note the early return: when no Flow is configured it still dispatches
     * `summarychange`. That is deliberate — the parent clears `_bgJobs.summary` on
     * that event, so without it the tab loader would spin forever and the CTA would
     * stay a no-op.
     */
    ...
    /**
     * Derive action items from the same meeting inputs as the summary.
     *
     * Reuses the summary's `flowParams` verbatim so the two AI calls see identical
     * context. The response is expected to be a JSON array but is treated as
     * untrusted: ```-fences are stripped, both a bare array and `{todos: [...]}` are
     * accepted, each item's keys are normalised across PascalCase/camelCase/snake_case,
     * and a parse failure degrades to an empty list rather than an error.
     *
     * Each to-do is decorated with UI bookkeeping (`_key`, `_accepted`, `_rowClass`,
     * `_hasMeta`) which is stripped again before the Tasks are saved.
     *
     * The `finally` block emits `todoschange` on BOTH success and failure — the
     * parent clears `_bgJobs.todos` on that event, so skipping it on the error path
     * would leave the To-Do's tab loading forever.
     *
     * @param {object} flowParams the payload built by handleGenerateSummaryDirect
     */
    async _generateTodos(flowParams) {
        ...
        try {
            ...
            // Always emit completion — success or failure — so the parent's bg-job flag clears.
            this.dispatchEvent(new CustomEvent('todoschange', {
                bubbles: false,
                detail: { todos: this._meetingTodos, acceptedCount: this.acceptedTodosCount }
            }));
        }
    }
    ...
    handleMeetingTypeRadio(event) {
        const type = event.target.value;
        if (!type) return;
        this._selectedMeetingType = type;
        this._dispatchMeetingTypeOutputs();
    }
    ...
    get showPanelRatioToggle()   { return !this._leftPanelCollapsed && this.hasSummaryForEvent && !this.embeddedLayout; }
    ...
    // ╔═══════════════════════════════════════════════════════════════════════╗
    // ║ INHERITED WEALTH-PLAN CODE — not used by the meeting summary           ║
    // ╠═══════════════════════════════════════════════════════════════════════╣
    // ║ Everything from here to the Undo/Redo section is the wealth-plan       ║
    // ║ review grid the monolith carried: AI-JSON parsing, the render model,   ║
    // ║ existing-record merge and diff, row edit/lock/delete, duplicate        ║
    // ║ detection, the save preview and `saveWealthPlan`.                      ║
    // ║                                                                       ║
    // ║ It came along when this component was forked out of wealthPlanHelper.  ║
    // ║ The live wealth plan is advisorWealthPlan, mounted separately by the    ║
    // ║ parent. Nothing in advisorMeetingSummary.html renders these sections.  ║
    // ║ It is also what the 17 @wire adapters above exist for.                 ║
    // ║                                                                       ║
    // ║ Work on the wealth plan in advisorWealthPlan.js, not here.             ║
    // ╚═══════════════════════════════════════════════════════════════════════╝

    /**
     * Turns the AI's raw response into review-grid sections, defensively.
     * The model is prompted for JSON but returns it wrapped in prose, ```-fences,
     * smart quotes or a BOM. Cleanup ladder: strip noise → keep any prose before the
     * first bracket as the summary → truncate at the last matching bracket → parse.
     * On failure it shows the raw text and falls back to empty sections rather than
    ...
    @track _reviewSections = [];

    _refreshReviewSections() {
        this._reviewSections = this._buildReviewSections();
    }
    ...
    /**
     * Build the complete render model for the review grid.
     *
     * Joins `_sections` (records), SECTIONS (field metadata), the resolved option
     * lists, and the per-row UI state maps into one array the template renders with
     * no logic of its own — per field: widget flags, options, validity, error text;
     * per row: CSS class, source badge, title/subtitle/meta, diff fields; per
     ...
     * sections — a household with hundreds of records would otherwise recompute
     * every collapsed row on each keystroke.
     *
     * Memoised into `_reviewSections` and refreshed explicitly by
     * `_refreshReviewSections()`. If a mutation does not show up in the UI, the
     * usual cause is a missing `_refreshReviewSections()` call.
     */
        ...
        return this._sections.map((s, idx) => {
            ...
            const displayRecords = isExpanded ? s.records.map((rec, ri) => {
                ...
                return {
                    ...
                    _hasErrors: !rec._completed && hasRowErrors,
                    _canLock: !rec._completed && !hasRowErrors && !isRowLocked,
    ...
    get lockedRecordCount() {
        ...
        for (const s of this._sections) {
            ...
            for (const r of s.records) {
                if (r._completed || sectionLocked || !!this._lockedRows[r._id]) count++;
            }
        }
        return count;
    }
    ...
    handleBulkLockSection(event) {
        ...
        if (!sec) return;
        const optMap = this._cachedSelectOptionsMap;
    ...
    async _invokeWpSaveSubflow(preview) {
        ...
        try {
            ...
            // Track internal _ids being saved/deleted per section for partial-success marking
            const savedIdsBySection   = {};
            const deletedIdsBySection = {};

            const optMap = this._getFullOptMap();
            const route = (key) => {
                const secSaved   = new Set();
                const secDeleted = new Set();
                savedIdsBySection[key]   = secSaved;
                deletedIdsBySection[key] = secDeleted;
                ...
                const clean = r => {
                    ...
                    def.fields.forEach(f => {
                        if (r[f.api] == null) return;
                        if (f.type === 'select' && f.selectKey && r[f.api] !== '') {
                            const rawVal = String(r[f.api]);
                            const opts = optMap[f.selectKey] || [];
                            const lc = rawVal.trim().toLowerCase();
                            const match = opts.find(o =>
                                o.value === rawVal ||
                                o.value.trim().toLowerCase() === lc ||
                                (o.label && o.label.trim().toLowerCase() === lc)
                            );
                            c[f.api] = match ? match.value : rawVal;
                        } else {
                            c[f.api] = r[f.api];
                        }
                    });
            ...
            // Parse structured response — Apex returns 'Saved' on full success or
            // JSON { errors: [...], successSections: [...] } on partial/full failure
            let successSections = Object.keys(savedIdsBySection);
            let errorMessages   = [];
            if (resultStr && resultStr !== 'Saved') {
                try {
                    const parsed    = JSON.parse(resultStr);
                    successSections = parsed.successSections || [];
                    errorMessages   = parsed.errors || [];
                } catch (_) {
                    successSections = [];
                    errorMessages   = [resultStr];
                }
            }

            // Build the effective saved/deleted sets from successful sections only
            const allSavedIds   = new Set();
            const allDeletedIds = new Set();
            for (const key of successSections) {
                (savedIdsBySection[key]   || new Set()).forEach(id => allSavedIds.add(id));
                (deletedIdsBySection[key] || new Set()).forEach(id => allDeletedIds.add(id));
            }

            this._applySaveResult(allSavedIds, allDeletedIds, preview, successSections);

            if (errorMessages.length > 0) {
                this._wpSaveErrors = errorMessages;
            }
        } catch (e) {
    ...
    _applySaveResult(savedIds, deletedIds, preview, successSections = null) {
        ...
        if (successSections === null) {
            // Full success — clear all pending state
            this._lockedRows     = {};
            this._lockedSections = {};
            this._editedRows     = {};
            this._deletedRows    = {};
        } else {
            // Partial success — only clear state for records that actually saved/deleted
            const allHandled = new Set([...savedIds, ...deletedIds]);
            const updLocked  = { ...this._lockedRows };
            const updEdited  = { ...this._editedRows };
            const updDeleted = { ...this._deletedRows };
            allHandled.forEach(id => {
                delete updLocked[id];
                delete updEdited[id];
                delete updDeleted[id];
            });
            this._lockedRows = updLocked;
            this._editedRows = updEdited;
            this._deletedRows = updDeleted;
            // Keep section locks only for sections that failed
            const newLockedSecs = {};
            Object.entries(this._lockedSections).forEach(([k, v]) => {
                if (!successSections.includes(k)) newLockedSecs[k] = v;
            });
            this._lockedSections = newLockedSecs;
        }
            ...
            const cleanRec = r => {
                ...
                def.fields.forEach(f => {
                    ...
                    if (f.type === 'select' && f.selectKey && r[f.api] !== '') {
                        ...
                        clean[f.api] = match ? match.value : rawVal;
                    } else {
                        clean[f.api] = r[f.api];
                    }
                });
