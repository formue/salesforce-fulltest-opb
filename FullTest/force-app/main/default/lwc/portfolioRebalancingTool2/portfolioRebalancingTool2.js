...
export default class PortfolioRebalancingTool2 extends LightningElement {
    ...
    _getDataHash() {
        try {
            return JSON.stringify({ total: this.total, weights: this.weights, values: this.values, activeTab: this.activeTab, actLen: Object.keys(this.actualAmounts || {}).length, trades: this.manualTrades, overrides: this.manualOverrides });
        } catch (e) { return null; }
    }
    ...
    // ── HERO INPUT ─────────────────────────────────────────────────
    get formattedTotalValue() { return this.total > 0 ? this.total.toLocaleString('nb-NO') : ''; }
    ...
    get portfolioDrift() {
        ...
        return (totalDrift / ASSETS.length).toFixed(2).replace('.', ',') + '%';
    }
    ...
    get detailedAssetAllocation() {
        return ASSETS.map(a => {
            ...
            return {
                ...
                targetPct:    targetPct > 0 ? targetPct.toFixed(1).replace('.', ',') + ' %' : '—',
                boundary:     targetPct > 0 ? `${Math.max(0, targetPct - BOUNDARY_LIMIT).toFixed(1)}% – ${Math.min(100, targetPct + BOUNDARY_LIMIT).toFixed(1)}%` : '—',
                targetAmount: targetAmt  > 0 ? 'kr ' + Math.round(targetAmt).toLocaleString('nb-NO') : '—',
                marketValue:  actualValue > 0 ? 'kr ' + Math.round(actualValue).toLocaleString('nb-NO') : '—',
                actualPct:    actualValue > 0 ? actualPct.toFixed(2).replace('.', ',') + ' %' : '—',
                deviation:    targetPct > 0 ? (deviation >= 0 ? '+' : '') + deviation.toFixed(1).replace('.', ',') + ' %' : '—',
                deviationClass
            };
        });
    }
    ...
    get totalTargetPct() {
        return ASSETS.reduce((s, a) => s + (this.weights[a.key] || 0), 0).toFixed(1).replace('.', ',') + ' %';
    }
    get totalTargetAmount() {
        let t = 0;
        ASSETS.forEach(a => { if (this.total > 0) t += this.total * (this.weights[a.key] || 0) / 100; });
        return t > 0 ? 'kr ' + Math.round(t).toLocaleString('nb-NO') : '—';
    }
    get totalMarketValue() { return this.total > 0 ? 'kr ' + Math.round(this.total).toLocaleString('nb-NO') : '—'; }
    ...
    _mapFlowResultsToAssets(holdings) {
        ...
        try {
            ...
            this.values = nv;
            this.total  = newTotal;
            Object.keys(nv).forEach(k => { this.valueInputs[k] = nv[k] > 0 ? nv[k].toLocaleString('nb-NO') : ''; });
            ...
            ASSETS.forEach(a => {
                ...
                planned.forEach(p => {
                    ...
                    productTrades.push({
                        ...
                        targetPctDisplay: tPct  !== null ? tPct.toFixed(1).replace('.', ',')  + ' %' : '—',
                        rawTargetPctDisplay: p.targetPct > 0 ? p.targetPct.toFixed(1).replace('.', ',') + ' %' : '—',
                        ...
                        postTradePctDisplay: ptPct !== null ? ptPct.toFixed(1).replace('.', ',') + ' %' : '—',
    ...
    _fmtKr(n)          { return 'kr ' + Math.round(n).toLocaleString('nb-NO'); }
    _fmtTrade(n)        { return (n >= 0 ? '+' : '') + 'kr ' + Math.round(n).toLocaleString('nb-NO'); }
    _fmtDisplayAmt(n)   { const v = this.roundToThousand ? Math.round(n / 1000) * 1000 : Math.round(n); return v.toLocaleString('nb-NO'); }
    ...
    get comparisonSections() {
        ...
        return ASSETS.map(a => {
            ...
            return {
                ...
                sumTargetDisplay: sumTarget > 0 ? 'kr ' + Math.round(sumTarget).toLocaleString('nb-NO') : '—',
                sumActualDisplay: sumActual > 0 ? Math.round(sumActual).toLocaleString('nb-NO') : '0',
                sumDevKrDisplay:  devKr !== 0 ? 'kr ' + (devKr >= 0 ? '+' : '') + Math.round(devKr).toLocaleString('nb-NO') : '—',
    ...
    _buildCmpRow(p) {
        ...
        return {
            ...
            targetDisplay: p.targetAmount > 0 ? 'kr ' + Math.round(p.targetAmount).toLocaleString('nb-NO') : '—',
            actualDisplay: actual !== null ? Math.round(actual).toLocaleString('nb-NO') : '',
            devKrDisplay:  devKr  !== null ? 'kr ' + (devKr >= 0 ? '+' : '') + Math.round(devKr).toLocaleString('nb-NO') : '—',
            ...
            targetPctDisplay: p.targetPct > 0 ? p.targetPct.toFixed(1).replace('.', ',') + ' %' : '—',
            actualPctDisplay: actualPct > 0 ? actualPct.toFixed(1).replace('.', ',') + ' %' : '—'
        };
    }
    ...
    _drawBeforeAfterChart() {
        ...
        data.forEach((d, i) => {
            ...
            ctx.font = '10px sans-serif'; ctx.fillStyle = 'rgba(160,170,190,0.9)'; ctx.textAlign = 'right';
            ctx.fillText(d.label, labelW - 6, yMid + 4);
            ctx.fillStyle = 'rgba(180,185,200,0.3)'; ctx.fillRect(labelW, top1, Math.max((d.actualPct / maxPct) * chartW, 2), barH);
            ...
            ctx.strokeStyle = 'rgba(220,230,255,0.55)'; ctx.lineWidth = 1.5; ctx.stroke(); ctx.restore();
        });
    }
    ...
    _drawChangeChart() {
        ...
        ctx.beginPath(); ctx.moveTo(cx, 0); ctx.lineTo(cx, h); ctx.strokeStyle = 'rgba(128,128,128,0.3)'; ctx.lineWidth = 1; ctx.stroke();
        ...
        data.forEach((d, i) => {
            ...
            ctx.font = '10px sans-serif'; ctx.fillStyle = 'rgba(160,170,190,0.85)';
    ...
    // ── AI ─────────────────────────────────────────────────────────
    handleGenerateAI() {
        if (!this.promptTemplateApiName) { console.warn('AI Flow API Name not configured.'); return; }
    ...
    handleHeroInputBlur(event)  { const v = parseFloat(event.target.value.replace(/\s/g, '')) || 0; event.target.value = v > 0 ? v.toLocaleString('nb-NO') : ''; }
    handleFocusNum(event)       { event.target.value = event.target.value.replace(/\s/g, ''); }
    handleBlurNum(event)        { const v = parseThousands(event.target.value); if (v !== null && !isNaN(v)) event.target.value = Math.round(v).toLocaleString('nb-NO'); }
    ...
    handleCmpActualBlur(event) { const sym = event.currentTarget.dataset.symbol, raw = this.actualAmounts[sym]; event.target.value = raw !== undefined ? Math.round(raw).toLocaleString('nb-NO') : ''; }
